...
//                    }
//                }
//                Student[] JuStu = new Student[size];
                for (int i = 0; i < Students.length; i++) 
                {
                     if(Students[i].getRegistrationDate().getYear() == 2013)
                     {
    ...
    public static void menuOption4(){
        ...
        for(int i=0;i<Students.length;i++)
        {
            if(Students[i] instanceof FullTimeStudent)
            {
                while(tempNum == ((FullTimeStudent)Students[i]).getStudentNumber())
                {
                    System.out.println("Please enter the part time student's number:");
                    tempNum = k.nextInt();
                }
            }
            if(Students[i] instanceof ParTimeStudent)
            {
                if(tempNum == ((ParTimeStudent)Students[i]).getStudentNumber())
                {
